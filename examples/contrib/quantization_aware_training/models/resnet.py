...
from utils.utilities import qconv2d, qrelu
...
def conv1x1(
    in_planes, out_planes, stride=1, norm=False, act=False, qat_mode=False, infer=False
):
    """1x1 convolution"""
    return qconv2d(
        in_channels=in_planes,
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = conv3x3(
            inplanes, planes, stride, norm=True, qat_mode=qat_mode, infer=infer
        )
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(
            block, 64, layers[0], qat_mode=qat_mode, infer=infer
        )
    ...
    def _make_layer(
        self, block, planes, blocks, stride=1, dilate=False, qat_mode=False, infer=False
    ):
