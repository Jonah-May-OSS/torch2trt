...
def main():
    ...
    print(f"===>> Output folder = {dir_name}")
    ...
    if args.m == "resnet18":
        model = resnet18(qat_mode=True) if args.netqat else resnet18()
    elif args.m == "resnet34":
        model = resnet34(qat_mode=True) if args.netqat else resnet34()
        ...
        raise NotImplementedError(f"model {args.m} is not defined")
    ...
    optimizer = optim.SGD(
        filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, momentum=0.9
    )
    if args.load_ckpt:
        ...
        print(
            f"===>>> Checkpoint loaded successfully from {args.load_ckpt} at epoch {epoch} "
        )
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.num_epochs):
        ...
        print(
            f"Epoch: {epoch + 1} | Loss: {np.around(running_loss, 6)} | Test accuracy: {test_accuracy}| Time Taken (sec): {np.around((end - start), 4)} "
        )
