...
from torch2trt.contrib.qat.layers.quant_activation import IQuantReLU, QuantReLU
...
def mapping_names(state_dict):
    ...
    for k, v in state_dict.items():
        if re.search(r"conv\d.weight", k):
            ...
            print(f"adding {k} to the new list")
            new_list[k] = v
    return new_list
    ...
    for k in model_state:
        if k not in model_state_dict:
            if re.search(r"folded_weight", k):
                ...
                print(f"adding {k} with shape {tensor_size} to the model state dict")
            elif re.search(r"folded_bias", k):
                item = re.sub("folded_bias", "weight", k)
                tensor_size = model_state[item].size()
                model_state_dict[k] = torch.ones(tensor_size[0])
                print(f"adding {k} with shape {tensor_size[0]} to the model state dict")
            else:
                print(f"adding {k} to the model state dict")
...
def printStats(graphName, timings, batch_size):
    ...
    msg = (
        f"\n{graphName} =================================\n"
        f"batch size={batch_size}, num iterations={steps}\n"
        f"  Median FPS: {speed_med:.1f}, mean: {speed_mean:.1f}\n"
        f"  Median latency: {time_med:.6f}, mean: {time_mean:.6f}, "
        f"99th_p: {time_99th:.6f}, std_dev: {time_std:.6f}\n"
    )
    print(msg)
