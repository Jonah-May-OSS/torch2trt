...
if len(detector_dataset) == 0:
    raise ValueError(
        "Detector dataset is empty, make sure to run generate_data.py first."
    )

if len(recognizer_dataset) == 0:
    raise ValueError(
        "Recognizer dataset is empty, make sure to run generate_data.py first."
    )
...
if args.int8:
    ...
    for i in range(num_calib):
        calib_dataset.insert(
            tuple(
                [
                    t.float() + 0.2 * torch.randn_like(t.float())
                    for t in recognizer_dataset[i % len(recognizer_dataset)]
                ]
            )
        )
...
def profile_module(module, dataset, count=None):
    ...
    module(*dataset[0])  # warmup

    torch.cuda.current_stream().synchronize()
    t0 = time.monotonic()
    for i in range(count):
        module(*dataset[i % len(dataset)])
    torch.cuda.current_stream().synchronize()
    t1 = time.monotonic()
