...
torch2trt = SourceFileLoader(
    "torch2trt", "torch2trt/__init__.py"
).load_module()  # to load relative to root
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--github", type=str, default="https://github.com/NVIDIA-AI-IOT/torch2trt"
    )
    ...
    for method, entry in torch2trt.CONVERTERS.items():
        ...
        url = "{github}/blob/{commit}/{relpath}#L{lineno}".format(
            ...
            relpath=os.path.relpath(
                converter.__code__.co_filename, os.path.abspath(".")
            ),
            lineno=converter.__code__.co_firstlineno,
        )

        print(f"| ``{method}`` | [``{converter.__name__}``]({url}) |")
