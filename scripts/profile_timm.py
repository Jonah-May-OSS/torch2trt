...
from torch2trt import TRTModule, torch2trt, trt
...
class Args:
    ...
    log_level: Literal["verbose", "info", "error", "warning", "internal_error"] = (
        "error"
    )
    use_onnx: bool = False
    ...
    def run(self):
        ...
        with (
            open(self.stderr_filepath(), "w") as ferr,
            open(self.stdout_filepath(), "w") as fout,
            ...
            latency_torch = profile_latency(
                model, data, self.num_warmup, self.num_profile
            )
            ...
            latency_trt = profile_latency(
                model_trt, data, self.num_warmup, self.num_profile
            )
