...
def cross_validate(module, inputs, fp16_mode: bool, tol: float):
    ...
    module_trt = torch2trt.torch2trt(module, inputs, fp16_mode=fp16_mode)
    ...
    for output_tensor, output_tensor_trt in zip(output, output_trt, strict=False):
        assert torch.allclose(output_tensor, output_tensor_trt, atol=tol, rtol=tol)
...
@pytest.mark.parametrize(
    "output_size,fp16_mode,tol",
    [((1, 1), False, 1e-1), ((2, 2), False, 1e-1), ((1, 1), True, 1e-1)],
)
def test_adaptive_avg_pool2d(output_size, fp16_mode, tol):
    module = (
        UnaryModule(lambda x: torch.nn.functional.adaptive_avg_pool2d(x, output_size))
        .cuda()
        .eval()
    )
...
@pytest.mark.parametrize(
    "output_size,fp16_mode,tol",
    [((1, 1, 1), False, 1e-1), ((2, 2, 2), False, 1e-1), ((1, 1, 1), True, 1e-1)],
)
def test_adaptive_avg_pool3d(output_size, fp16_mode, tol):
    module = (
        UnaryModule(lambda x: torch.nn.functional.adaptive_avg_pool3d(x, output_size))
        .cuda()
        .eval()
    )
...
def test_adaptive_max_pool2d(output_size, fp16_mode, tol):
    module = (
        UnaryModule(lambda x: torch.nn.functional.adaptive_max_pool2d(x, output_size))
        .cuda()
        .eval()
    )
    inputs = [torch.randn(1, 3, 4, 4).cuda()]
...
def test_adaptive_max_pool3d(output_size, fp16_mode, tol):
    module = (
        UnaryModule(lambda x: torch.nn.functional.adaptive_max_pool3d(x, output_size))
        .cuda()
        .eval()
    )
    inputs = [torch.randn(1, 3, 4, 4, 4).cuda()]
...
def test_batch_norm_nd(nd, with_conv):
    ...
    if nd == 1:
        if with_conv:
            modules.append(
                nn.Conv1d(3, 3, 1)
            )  # with conv, because scale layer not implemented sometimes.
...
@pytest.mark.parametrize("chunks,dim", [(1, 1), (3, 1)])
...
@pytest.mark.parametrize("split_sections_or_size,dim", [(1, 1), ([1, 2], 1), (1, -1)])
def test_split_sections(split_sections_or_size, dim):
    module = (
        UnaryModule(lambda x: torch.split(x, split_sections_or_size, dim)).cuda().eval()
    )
    inputs = [torch.randn(1, 3, 3).cuda()]
    cross_validate(module, inputs, fp16_mode=False, tol=1e-1)


@pytest.mark.parametrize("min,max", [(None, 0.5), (-0.5, None), (-0.5, 0.5)])
...
@pytest.mark.parametrize("nd", [1, 2, 3])
def test_conv(nd, kernel_size, stride, padding, dilation, groups, bias, fp16_mode):
    if nd == 1:
        cls = nn.Conv1d
    elif nd == 2:
        cls = nn.Conv2d
...
def test_conv_transpose(
    nd, kernel_size, stride, padding, dilation, groups, bias, fp16_mode
):
    if nd == 1:
...
@pytest.mark.parametrize("val", [2, 2.0])
...
@pytest.mark.parametrize("einsum_expr", ["bij,bjk->bik"])
...
@pytest.mark.parametrize("sizes", [(3, 4), (-1, 4)])
...
@pytest.mark.parametrize("start_dim,end_dim", [(0, -1), (1, -1), (1, 3)])
...
@pytest.mark.parametrize("denom", [1.0, 2.0])
...
@pytest.mark.parametrize(
    "num_groups,num_channels,affine", [(1, 6, False), (3, 6, False), (3, 6, True)]
)
...
@pytest.mark.parametrize("input_shapes", [(1, 4)])
...
@pytest.mark.parametrize(
    "shape_a,shape_b", [((1, 2, 3), (1, 3, 4)), ((2, 2, 3), (2, 3, 4))]
)
...
def test_max_pool_nd(nd, kernel_size, stride, padding, dilation, ceil_mode):
    if nd == 1:
        ...
        cls = nn.MaxPool3d
    module = (
        cls(kernel_size, stride, padding, dilation, ceil_mode=ceil_mode).cuda().eval()
    )
    input_size = [1, 3] + [4] * nd
    inputs = [torch.randn(*input_size).cuda()]
    cross_validate(module, inputs, fp16_mode=False, tol=1e-1)
...
@pytest.mark.parametrize("dim,start,length", [(0, 0, 2), (1, 1, 2), (-1, -1, 1)])
...
def test_normalize(p, dim):
    module = (
        UnaryModule(lambda x: torch.nn.functional.normalize(x, p, dim)).cuda().eval()
    )
    inputs = [torch.zeros(1, 3, 3).cuda()]
    cross_validate(module, inputs, fp16_mode=False, tol=1e-1)
...
@pytest.mark.parametrize(
    ...
    [
        ((1, 1), "constant", 0.0),
        ((1, 1, 2, 2), "constant", 0.0),
        ((0, 1, 2, 1, 3, 3), "constant", 0.0),
    ],
)
def test_pad(pad, mode, value):
    module = (
        UnaryModule(lambda x: torch.nn.functional.pad(x, pad, mode, value))
        .cuda()
        .eval()
    )
...
@pytest.mark.parametrize("permutation", [(0, 2, 1), (0, 2, 1, 3)])
def test_permute(permutation):

    module = UnaryModule(lambda x: x.permute(*permutation)).cuda().eval()
