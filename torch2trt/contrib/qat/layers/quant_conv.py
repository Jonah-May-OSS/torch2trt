...
class QuantConv2d(_QuantConvNd):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        quant_desc_weight = _utils.pop_quant_desc_in_kwargs(
            self.__class__, weight_only=True, **kwargs
        )
        super().__init__(
            in_channels,
            out_channels,
...
class QuantConvBN2d(_QuantConvNd):
    ...
    def _fold_BN(self, conv_w, conv_b, bn_rm, bn_rv, eps, bn_w, bn_b):
        ...
        self.folded_weight = conv_w * (scale_factor).reshape(
    ...
    def forward(self, input):
        # the actual quantization happens in the next level of the class hierarchy
        folded_weight, folded_bias = self._fold_BN(
            self.weight,
            self.bias,
