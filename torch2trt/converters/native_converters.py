...
def convert_selu(ctx):
    input = get_arg(ctx, "input", pos=0, default=None)
    get_arg(ctx, "alpha", pos=1, default=1.0)
    output = ctx.method_return
...
def convert_batch_norm(ctx):
    ...
    layer = ctx.network.add_scale_nd(
        scale_input, trt.ScaleMode.CHANNEL, bias, scale, power, 1
    )
...
def _add_clamp_val(network, trt_input, val, op):
    ...
    val_tensor = (
        val
        * torch.ones(val_shape, dtype=torch_dtype_from_trt(trt_input.dtype))
        .cpu()
        .numpy()
    )
...
def _add_clamp_tensor(network, trt_input, tensor, op):
    ...
    trt_input, tensor_trt = broadcast_trt_tensors(
        network,
        [trt_input, tensor_trt],
...
def __add_clamp(network, trt_input, val, op):
    return (
        _add_clamp_tensor(network, trt_input, val, op)
...
def convert_clamp(ctx):
    ...
    if min_val is not None and max_val is not None:
        layer = __add_clamp(
            ctx.network, input_trt, min_val, trt.ElementWiseOperation.MAX
        )
        layer = __add_clamp(
            ctx.network, layer.get_output(0), max_val, trt.ElementWiseOperation.MIN
        )
        ...
        layer = __add_clamp(
            ctx.network, input_trt, max_val, trt.ElementWiseOperation.MIN
        )
    else:
...
def convert_div(ctx):
    ...
    layer = ctx.network.add_elementwise(
        input_a_trt, input_b_trt, trt.ElementWiseOperation.DIV
    )
    output._trt = layer.get_output(0)
...
def convert_einsum(ctx):
    ...
    layer = ctx.network.add_einsum([t._trt for t in input_tensors], einsum_eq)
...
def convert_expand(ctx):
    ...
    start = tuple([0] * ndim)
    stride = tuple(
        [int(i == o) for i, o in zip(inshape, shape, strict=False)]
...
def convert_flatten(ctx):
    ...
    if start_dim != end_dim:
        new_shape_trt.append(
            ctx.network.add_constant(
                [1], np.array([-1], dtype=trt_int_dtype())
            ).get_output(0)
        )
...
def convert_floordiv(ctx):
    ...
    ab_layer = ctx.network.add_elementwise(
    ...
    sign_layer = ctx.network.add_elementwise(
        ab_layer.get_output(0), abs_ab_layer.get_output(0), trt.ElementWiseOperation.DIV
    )
    ...
    abs_floor_layer = ctx.network.add_elementwise(
        abs_a_layer.get_output(0),
        abs_b_layer.get_output(0),
        trt.ElementWiseOperation.FLOOR_DIV,
    )
    out_layer = ctx.network.add_elementwise(
        sign_layer.get_output(0),
        abs_floor_layer.get_output(0),
...
def convert_gelu(ctx):
    ...
    x, c05, c1, cs2pi, c044, c3 = add_missing_trt_tensors(
        ctx.network, [input, 0.5, 1.0, math.sqrt(2.0 / math.pi), 0.044715, 3.0]
    )

    x, c05, c1, cs2pi, c044, c3 = broadcast_trt_tensors(
        ctx.network, [x, c05, c1, cs2pi, c044, c3], len(output.shape)
    )

    y = ctx.network.add_elementwise(x, c3, trt.ElementWiseOperation.POW).get_output(0)
    y = ctx.network.add_elementwise(y, c044, trt.ElementWiseOperation.PROD).get_output(
        0
    )
    y = ctx.network.add_elementwise(x, y, trt.ElementWiseOperation.SUM).get_output(0)
    y = ctx.network.add_elementwise(y, cs2pi, trt.ElementWiseOperation.PROD).get_output(
        0
    )
...
def convert_tensor_getitem(ctx):
    ...
    for s in slices:
        ...
        elif isinstance(s, (int, IntWrapper)):
            new_slices.append(s)
...
def convert_group_norm(ctx):
    ...
    a_diff = ctx.network.add_elementwise(
        a, a_mean, trt.ElementWiseOperation.SUB
    ).get_output(0)
    a_dist = ctx.network.add_elementwise(
        a_diff, a_diff, trt.ElementWiseOperation.PROD
    ).get_output(0)
    a_var = ctx.network.add_reduce(
        a_dist, trt.ReduceOperation.AVG, axes, keepdim
    ).get_output(0)

    a_var, eps_trt = broadcast_trt_tensors(
        ctx.network, [a_var, eps_trt], len(split_shape)
    )

    a_var_eps = ctx.network.add_elementwise(
        a_var, eps_trt, trt.ElementWiseOperation.SUM
    ).get_output(0)
    a_std = ctx.network.add_unary(a_var_eps, trt.UnaryOperation.SQRT).get_output(0)

    # divide by stdev
    b = ctx.network.add_elementwise(
        a_diff, a_std, trt.ElementWiseOperation.DIV
    ).get_output(0)
    ...
    if weight is not None or bias is not None:
        ...
        layer = ctx.network.add_scale_nd(
            c, trt.ScaleMode.CHANNEL, bias, scale, power, 1
        )
        c = layer.get_output(0)
...
def convert_instance_norm(ctx):
    ...
    get_arg(ctx, "momentum", pos=6, default=0.1)
    ...
    if not use_input_stats:
        ...
        result_trt = _add_scale_1d2d3d(
            ctx.network, input._trt, trt.ScaleMode.CHANNEL, offset, scale, power
        )
    ...
    # CASE 2 - USING INPUT STATS
        ...
        mean_trt = ctx.network.add_reduce(
            input._trt, trt.ReduceOperation.AVG, reduce_axes, keep_dims
        ).get_output(0)

        # compute variance over spatial (include eps, to reduce layer count)
        delta_trt = ctx.network.add_elementwise(
            input._trt, mean_trt, trt.ElementWiseOperation.SUB
        ).get_output(0)
        var_trt = ctx.network.add_scale(
            delta_trt,
            trt.ScaleMode.UNIFORM,
            np.zeros_like(eps_np),
            np.ones_like(eps_np),
            2 * np.ones_like(eps_np),
        ).get_output(0)
        var_trt = ctx.network.add_reduce(
            var_trt, trt.ReduceOperation.AVG, reduce_axes, keep_dims
        ).get_output(0)
        ...
        var_trt = ctx.network.add_scale(
            ...
            eps_np,
            np.ones_like(eps_np),
            0.5 * np.ones_like(eps_np),
        ).get_output(0)

        # compute final result
        result_trt = ctx.network.add_elementwise(
            delta_trt, var_trt, trt.ElementWiseOperation.DIV
        ).get_output(0)
...
def convert_interpolate(ctx):
    ...
    def configure_resize_trt_10(layer):
        if mode.lower() in ["linear", "bilinear", "trilinear"]:
            ...
            raise RuntimeError(
                f"Interpolation with mode={mode} is not supported by torch2trt."
            )
...
def convert_layer_norm(ctx):
    ...
    input_trt, eps_trt = broadcast_trt_tensors(
        ctx.network, [input_trt, eps_trt], len(output.shape)
    )

    if weight is not None:
        _, weight_trt = add_missing_trt_tensors(ctx.network, [input, weight])
        _, weight_trt = broadcast_trt_tensors(
            ctx.network, [input_trt, weight_trt], len(output.shape)
        )

    if bias is not None:
        _, bias_trt = add_missing_trt_tensors(ctx.network, [input, bias])
        _, bias_trt = broadcast_trt_tensors(
            ctx.network, [input_trt, bias_trt], len(output.shape)
        )
    ...
    ux = ctx.network.add_reduce(
        input_trt, trt.ReduceOperation.AVG, axes, keep_dims=True
    ).get_output(0)
    numerator = ctx.network.add_elementwise(
        input_trt, ux, trt.ElementWiseOperation.SUB
    ).get_output(0)
    varx = ctx.network.add_elementwise(
        numerator, numerator, trt.ElementWiseOperation.PROD
    ).get_output(0)
    varx = ctx.network.add_reduce(
        varx, trt.ReduceOperation.AVG, axes, keep_dims=True
    ).get_output(0)
    denom = ctx.network.add_elementwise(
        varx, eps_trt, trt.ElementWiseOperation.SUM
    ).get_output(0)
    denom = ctx.network.add_unary(denom, trt.UnaryOperation.SQRT).get_output(0)
    y = ctx.network.add_elementwise(
        numerator, denom, trt.ElementWiseOperation.DIV
    ).get_output(0)

    if weight is not None:
        y = ctx.network.add_elementwise(
            y, weight_trt, trt.ElementWiseOperation.PROD
        ).get_output(0)

    if bias is not None:
        y = ctx.network.add_elementwise(
            y, bias_trt, trt.ElementWiseOperation.SUM
        ).get_output(0)
...
def convert_matmul(ctx):
    ...
    layer = ctx.network.add_matrix_multiply(
        x_trt, trt.MatrixOperation.NONE, y_trt, trt.MatrixOperation.NONE
    )
...
def convert_max_pool_nd(ctx):
    ...
    get_arg(ctx, "dilation", pos=4, default=1)
...
def __convert_max_elementwise(ctx):
    ...
    layer = ctx.network.add_elementwise(
        input_a_trt, input_b_trt, trt.ElementWiseOperation.MAX
    )
    output._trt = layer.get_output(0)
...
def __convert_max_reduce(ctx):
    ...
    layer = ctx.network.add_reduce(
        input_trt, trt.ReduceOperation.MAX, torch_dim_to_trt_axes(dim), keepdim
    )
...
def __convert_min_elementwise(ctx):
    ...
    layer = ctx.network.add_elementwise(
        input_a_trt, input_b_trt, trt.ElementWiseOperation.MIN
    )
    output._trt = layer.get_output(0)
...
def __convert_min_reduce(ctx):
    ...
    if isinstance(ctx.method_return, torch.Tensor):
        ...
        output_val = ctx.method_return[0]
    layer = ctx.network.add_reduce(
        input_trt, trt.ReduceOperation.MIN, torch_dim_to_trt_axes(dim), keepdim
    )
    output_val._trt = layer.get_output(0)
...
def convert_fmod(ctx):
    ...
    # a % b  =  a - (a//b) * b
    floordiv_layer = ctx.network.add_elementwise(
        sign_layer.get_output(0),
        abs_floor_layer.get_output(0),
        trt.ElementWiseOperation.PROD,
    )
    prod_layer = ctx.network.add_elementwise(
        floordiv_layer.get_output(0), input_b_trt, trt.ElementWiseOperation.PROD
    )
    sub_layer = ctx.network.add_elementwise(
        input_a_trt, prod_layer.get_output(0), trt.ElementWiseOperation.SUB
    )
...
def convert_mod(ctx):
    ...
    # a % b  =  a - (a//b) * b
    floordiv_layer = ctx.network.add_elementwise(
        input_a_trt, input_b_trt, trt.ElementWiseOperation.FLOOR_DIV
    )
    prod_layer = ctx.network.add_elementwise(
        floordiv_layer.get_output(0), input_b_trt, trt.ElementWiseOperation.PROD
    )
    mod_layer = ctx.network.add_elementwise(
        input_a_trt, prod_layer.get_output(0), trt.ElementWiseOperation.SUB
    )
...
def convert_narrow(ctx):
    ...
    dim = (
        ctx.method_args[1]
        if get_arg(ctx, "dim", pos=1, default=0) >= 0
        else len(shape) + get_arg(ctx, "dim", pos=1, default=0)
    )
    ...
    layer = ctx.network.add_slice(
        input=input_trt, start=start, shape=shape, stride=stride
    )
    output._trt = layer.get_output(0)
...
def convert_ne(ctx):
    ...
    layer_1 = ctx.network.add_elementwise(
        input_a_trt, input_b_trt, trt.ElementWiseOperation.EQUAL
    )
...
def convert_normalize(ctx):
    ...
    input_trt, eps_trt, p_trt, p_inv_trt = add_missing_trt_tensors(
        ctx.network, [input, eps, p, 1.0 / p]
    )
    input_trt, eps_trt, p_trt, p_inv_trt = broadcast_trt_tensors(
        ctx.network, [input_trt, eps_trt, p_trt, p_inv_trt], len(input_trt.shape)
    )
    ...
    norm = ctx.network.add_elementwise(
        norm, p_trt, trt.ElementWiseOperation.POW
    ).get_output(0)
    norm = ctx.network.add_reduce(
        norm, trt.ReduceOperation.SUM, torch_dim_to_trt_axes(dim), keep_dims=True
    ).get_output(0)
    norm = ctx.network.add_elementwise(
        norm, p_inv_trt, trt.ElementWiseOperation.POW
    ).get_output(0)

    # clamp norm = max(norm, eps)
    norm = ctx.network.add_elementwise(
        norm, eps_trt, trt.ElementWiseOperation.MAX
    ).get_output(0)

    # divide input by norm
    output._trt = ctx.network.add_elementwise(
        input_trt, norm, trt.ElementWiseOperation.DIV
    ).get_output(0)
...
def convert_pad(ctx):
    ...
    get_arg(ctx, "mode", 2, "constant")
    get_arg(ctx, "value", 3, 0.0)
...
def convert_pow(ctx):
    ...
    layer = ctx.network.add_elementwise(
        input_a_trt, input_b_trt, trt.ElementWiseOperation.POW
    )
    output._trt = layer.get_output(0)
...
def convert_prelu(ctx):
    ...
    weight_trt = ctx.network.add_constant(
        weight_shape, -weight.detach().view(weight_shape).cpu().numpy()
    ).get_output(0)  # detach so considered leaf
    ...
    b = ctx.network.add_elementwise(
        b, weight_trt, trt.ElementWiseOperation.PROD
    ).get_output(0)
...
def convert_prod(ctx):
    ...
    output = ctx.method_return
    layer = ctx.network.add_reduce(
        input_trt, trt.ReduceOperation.PROD, torch_dim_to_trt_axes(dim), keepdim
    )
    output._trt = layer.get_output(0)
...
def convert_roll(ctx):
    ...
    for s, d in zip(shifts, dims, strict=False):
        start[d] = (-s) % shape[d]
...
def convert_silu(ctx):
    ...
    layer = ctx.network.add_activation(input_trt, trt.ActivationType.SIGMOID)
    layer = ctx.network.add_elementwise(
        input_trt, layer.get_output(0), trt.ElementWiseOperation.PROD
    )
...
def convert_sub(ctx):
    ...
    layer = ctx.network.add_elementwise(
        input_a_trt, input_b_trt, trt.ElementWiseOperation.SUB
    )
    output._trt = layer.get_output(0)
...
def convert_sum(ctx):
    ...
    output = ctx.method_return
    layer = ctx.network.add_reduce(
        input_trt, trt.ReduceOperation.SUM, torch_dim_to_trt_axes(dim), keepdim
    )
    output._trt = layer.get_output(0)
...
def convert_unsqueeze(ctx):
    ...
    new_shape_trt.insert(
        dim,
        ctx.network.add_constant([1], np.array([1], dtype=trt_int_dtype())).get_output(
            0
        ),
    )
