...
def _default_condition(x):
    return isinstance(x, torch.Tensor) and (
        x.dtype is torch.half or x.dtype is torch.float or x.dtype == torch.bool
    )
...
class Flattener:
    ...
    def _flatten(self, value, result):
        ...
        elif isinstance(self._schema, (list, tuple)):
            for child_value, child_schema in zip(value, self._schema, strict=False):
    ...
    def unflatten(self, flattened):
        if isinstance(self._schema, int):
            ...
            for child_key in sorted(self._schema.keys()):
                ...
                result[child_key] = Flattener(child_schema, self.size).unflatten(
                    flattened
                )
            return result
        else:
